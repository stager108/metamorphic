...
const double INDEL_PROBABILITY = 0.0005;
...
int add_replacements(int edge) {
    ...
    while ((fscanf(infile, "%c", &a) == 1) && (position < edge)) {
        ...
        if (p < INDEL_PROBABILITY) {
            ...
            fprintf(config, "%c", '\n');
            fprintf(inner_config, "%c", '\n');
            length = (rand() % MAX_SMALL_MUT_SIZE) + 1;
            k = 0;
            while (k < length) {
                if ((fscanf(infile, "%c", &a) != 1) || (position >= edge)) {
                    length = k;
    ...
    while (fscanf(infile, "%c", &a) == 1) {
        ...
        switch (opt) {
            ...
            case 'i':
                ...
                break;
            case 'r':
                add_replacements(position + length);
                break;
            case 'p':
...
int main(int argc, char *argv[]) {
    ...
    int opt;
    while ((opt = getopt(argc, argv, "idpr")) != -1) {
        switch (opt) {
            case 'i':
            case 'd':
            case 'r':
            ...
            default:
                ...
                fprintf(stderr, "-i insertions -d deletions -p point mutations -r replacements\n");
                exit(EXIT_FAILURE);
        }
    }
